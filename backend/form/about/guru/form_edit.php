...
<body>
    <?php
    $id = $_GET['id'];
    $sql = "SELECT * FROM guru where id = $id limit 1";
    $data = $conn->query($sql);
    if ($data->num_rows > 0) :
    ...
    ?>
            ...
            <div class="container">
                <form action="aksi_edit.php" method="POST" enctype="multipart/form-data">
                    ...
                    <br>
                    <label for="exampleInputEmail1" class="form-label">Masukan Instagram</label>
                    <input type="text" class="form-control" value="<?= $row['insta'] ?>" name="insta" placeholder="Masukan ">
                    <br>
                    <label for="exampleInputEmail1" class="form-label">Masukan Facebook</label>
                    <input type="text" class="form-control" value="<?= $row['fb'] ?>" name="fb" placeholder="Masukan ">
                    <br>
