...
if (isset($_POST['update'])) {
    ...
    if ($_FILES['foto']['error'] !== 4) {
        ...
        if (move_uploaded_file($file_tmp, $path . $nama_gambar)) {
            $sql = "UPDATE guru SET judul = '$judul',  nama = '$nama', keterangan = '$keterangan', insta = '$insta', fb = '$fb', foto = '$nama_gambar' where ID = $id";
            var_dump($sql);
...
?>
        ...
        $sql = "UPDATE guru SET judul = '$judul', nama = '$nama', keterangan = '$keterangan', insta = '$insta', fb = '$fb' where ID = $id";
        $berhasil = ($conn->query($sql) === true);
