...
<body>
    <?php
    $id = $_GET['id'];
    $sql = "SELECT * FROM progress where id = $id limit 1";
    $data = $conn->query($sql);
    if ($data->num_rows > 0) :
    ...
    ?>
            ...
            <div class="container">
                <form action="aksi_edit.php" method="POST" enctype="multipart/form-data">
                    ...
                    <br>
                    <label for="exampleInputEmail1" class="form-label">Masukan Nomer</label>
                    <input type="number" class="form-control" value="<?= $row['nomer'] ?>" name="nomer" placeholder="Masukan ">
                    <br>
