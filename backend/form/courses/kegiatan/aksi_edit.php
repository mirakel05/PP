...
if (isset($_POST['update'])) {
    ...
    if ($_FILES['foto']['error'] !== 4) {
        ...
        if (move_uploaded_file($file_tmp, $path . $nama_gambar)) {
            $sql = "UPDATE kegiatan SET judul = '$judul',  nama = '$nama', keterangan = '$keterangan', foto = '$nama_gambar' where ID = $id";
            var_dump($sql);
...
?>
        ...
        $sql = "UPDATE kegiatan SET judul = '$judul', nama = '$nama', keterangan = '$keterangan' where ID = $id";
        $berhasil = ($conn->query($sql) === true);
